  ...
  if (alerts.length === 0) {
    return (
      <div className="py-12 text-center">
        ...
        <p className="text-gray-600 text-sm">No alerts in the selected time range</p>
      </div>
    );
  }
      ...
      <table className="w-full">
        <thead className="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th className="text-left px-4 py-3 text-sm font-medium text-gray-600">Status Code</th>
        ...
        <tbody className="divide-y divide-gray-200">
          {alerts.map((alert) => (
            <tr key={alert.id}>
              ...
              <td className="px-4 py-3 text-sm text-gray-600">{alert.status_code || "-"}</td>
              <td className="px-4 py-3 text-sm text-gray-600">{new Date(alert.created_at).toLocaleString()}</td>
